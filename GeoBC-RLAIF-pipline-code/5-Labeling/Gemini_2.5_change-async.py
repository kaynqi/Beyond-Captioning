...
client = AsyncOpenAI(
    ...
    # 【修复】直接填入字符串，去除了原代码中致命的逗号
    api_key=os.getenv("OPENROUTER_API_KEY"),
    # 明确配置异步 HTTP 客户端，并挂载本地代理，加入长超时
...
class Config:
    ...
    # 【并发控制】OpenRouter 的 Gemini 并发可以设为 5
...
ANSWER_JSON = r"G:\ct\our_dataset\train\change_regions_merged.json"
KNOWLEDGE_MD = r"G:\ct\our_dataset\test\land-use.md"
OUTPUT_JSONL = r"output\benchmark_gemini_answer_change-train.jsonl"
...
async def call_gemini_with_retry_async(messages):
    """带重试机制的异步 API 调用，使用流式接收防网关超时断流"""
    response = await client.chat.completions.create(
        model=Config.MODEL_NAME,
        messages=messages,
        stream=True,    # 【核心】保留流式输出，防长链接静默断开
        temperature=0,  # 严格依据事实解析，设为0
        timeout=180
    )
...
async def main_async():
    print("[*] 开始执行 Gemini-2.5-Pro (Change) 数据标注任务 (异步并发断点续传模式)...")
    ...
    print(f"\n[*] 🎉 Gemini-2.5-Pro (Change) 异步批量数据标注脚本执行完毕！")
