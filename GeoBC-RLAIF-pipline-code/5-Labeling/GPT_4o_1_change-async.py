...
client = AsyncOpenAI(
    ...
    api_key="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",
    # 明确配置异步 HTTP 客户端，挂载您代码中指定的 7897 端口代理
    http_client=httpx.AsyncClient(
        transport=httpx.AsyncHTTPTransport(proxy="http://127.0.0.1:7890"),
        timeout=180.0
    )
)
...
class Config:
    ...
    # 【并发控制】OpenAI 系列模型在 OpenRouter 上的并发额度通常较好，建议设为 5 ~ 10
    MAX_CONCURRENT_REQUESTS = 10

# ============================
# 路径配置
# ============================
# FOLDER_2024 = r"E:\ct\our_dataset\test\change\2024_RGB"
# FOLDER_2025 = r"E:\ct\our_dataset\test\change\post"
# ANSWER_JSON = r"E:\ct\our_dataset\test\change\change_regions.json"
# KNOWLEDGE_MD = r"E:\ct\our_dataset\test\land-use.md"
# OUTPUT_JSONL = r"output11\benchmark_gpt41_answer_change-async-train.jsonl"

FOLDER_2024 = r"G:\ct\our_dataset\val\change\2024_RGB"
FOLDER_2025 = r"G:\ct\our_dataset\val\change\post"
ANSWER_JSON = r"G:\ct\our_dataset\val\change_regions_merged.json"
KNOWLEDGE_MD = r"G:\ct\our_dataset\test\land-use.md"
OUTPUT_JSONL = r"output11\benchmark_gpt41_answer_change-async-val.jsonl"
...
async def call_model_with_retry_async(messages):
    ...
    response = await client.chat.completions.create(
        ...
        messages=messages,
        stream=True,    # 【核心】保留流式输出，防静默断联
        temperature=0,  # 事实性推理建议 temperature 设为 0
        timeout=180
    )
...
async def main_async():
    print(f"[*] 开始执行 {Config.MODEL_NAME} (Change) 数据标注任务 (异步并发断点续传模式)...")
    ...
    print(f"\n[*] 🎉 {Config.MODEL_NAME} (Change) 异步批量数据标注脚本执行完毕！")
