...
class Config:
    ...
    # 【并发控制】OpenRouter 相对稳定，Gemini 并发可以设为 5
    MAX_CONCURRENT_REQUESTS = 20

# 【修正】修改为 No-Change 任务对应的数据集路径与输出文件
# FOLDER_2024 = r"G:\ct\our_dataset\val\nochange\2024_RGB"
# FOLDER_2025 = r"G:\ct\our_dataset\val\nochange\post"
# ANSWER_JSON = r"G:\ct\our_dataset\val\nochange_regions_merged.json"
# KNOWLEDGE_MD = r"G:\ct\our_dataset\test\land-use.md"
# OUTPUT_JSONL = r"G:\text\output11\benchmark_gemini_answer_nochange-val.jsonl"

FOLDER_2024 = r"G:\ct\our_dataset\train\nochange\2024_RGB"
FOLDER_2025 = r"G:\ct\our_dataset\train\nochange\post"
ANSWER_JSON = r"G:\ct\our_dataset\train\nochange_regions_merged.json"
KNOWLEDGE_MD = r"G:\ct\our_dataset\test\land-use.md"
OUTPUT_JSONL = r"G:\text\output11\benchmark_gemini_answer_nochange-val.jsonl"
...
async def call_gemini_with_retry_async(messages):
    """带重试机制的异步 API 调用，使用流式接收防 OpenRouter 网关超时"""
    response = await client.chat.completions.create(
        model=Config.MODEL_NAME,
        messages=messages,
        stream=True,    # 保留流式输出防静默断联
        temperature=0,  # Gemini 任务建议 temperature 设为 0
        timeout=180
    )
...
async def process_single_task(prefix, img_pre_path, img_post_path, prompt_content, semaphore, write_lock, counter_info, output_jsonl_abs):
    ...
    async with semaphore:
        try:
            ...
            full_content = await call_gemini_with_retry_async(messages)
...
async def main_async():
    # 【修正】打印提示已修改为 No-Change
    print("[*] 开始执行 Gemini-2.5-Pro (No-Change) 数据标注任务 (异步并发断点续传模式)...")
    ...
    # 【修正】打印提示已修改为 No-Change
    print(f"\n[*] 🎉 Gemini-2.5-Pro (No-Change) 异步批量数据标注脚本执行完毕！")
